...
export class GGraph implements IGraph {
    private nodesMap: StringMap<GNodeInternal>; // id -> GNodeInternal
    private edgesMap: StringMap<GEdge>; // id -> GEdge
    nodes: GNode[];
    edges: GEdge[];
    boundingBox: GRect;
    settings: GSettings;
    ...
    beginLayoutGraph(callback: () => void = null): void {
        ...
        var that = this;
        var workerCallback = function (gstr: any) {
        ...
        //This uri format and method is used to avoid the cdn for the worker, web workers must be on the same origin as host.
        this.worker = new Worker('./Scripts/MSAGL/workerBoot.js');
